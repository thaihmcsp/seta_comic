...
from sqlalchemy.ext.asyncio import AsyncSession
from app.db.deps import get_session
from ..services.chapterService import (
    getChaptersOfComic,
    getOneChapterData,
    createChapters,
    updateChapters,
...
async def createChaptersAPI(
    ...
    if "guest" in current_user.role:
        raise HTTPException(status_code=403, detail="Unauthorized: Guest role cannot create chapters")
...
async def updateChaptersAPI(
    ...
    if "guest" in current_user.role:
        raise HTTPException(status_code=403, detail="Unauthorized: Guest role cannot update chapters")
...
async def deleteChapterAPI(
    ...
    if "guest" in current_user.role:
        raise HTTPException(status_code=403, detail="Unauthorized: Guest role cannot delete chapters")
...
async def getChapterById(chapter_id: int, session: AsyncSession = Depends(get_session)):
